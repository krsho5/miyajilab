...
int main(){
	...
	if(check==192){
		...
		mpz_init_set_str(pre_BY, P_192_PRE_BY, 10);
		loop = P_192_LOOPS_SICTMI;
		fltloop = P_192_LOOPS_FLT;
		byloop = P_192_LOOPS_BY;
		mpz_init_set_str(x1, P_192_bos, 10);
	}else if(check==224){
	...
	if(num==0){
		...
		CC1 = GetCC();
		for(int i = 0; i < N; ++i){
			BY(result,test_num[i], p, pre_BY, byloop);
		}
		CC2 = GetCC();
			printf("BY\n%ld\n", (CC2-CC1)/N);
		...
		CC1 = GetCC();
		for(int i = 0; i < N; ++i){
			BOS(result,test_num[i], p, loop, x1);
		}
		CC2 = GetCC();
			printf("BOS\n%ld\n", (CC2-CC1)/N);
